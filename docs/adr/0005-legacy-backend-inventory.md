...
프론트엔드에서 이들을 부르던 화면은 2026-09-06에 삭제됐습니다.
...
- `workers/src/`에 죽은 파일 4개(`practice-service.js`, `story-service.js`,
  `challenge-service.js`, `timefold-service.js`)가 계속 보입니다. 워커 번들에도 포함됩니다
  (73 KiB / gzip 16.75 KiB — 서버 사이드라 사용자 비용은 아님).
- D1에 안 쓰는 테이블 12개가 남습니다.
- **인증 없는 죽은 라우트가 공개돼 있습니다.** 지금은 읽기 위주라 위험이 낮지만,
  `POST /api/stories/seed` 같은 쓰기 라우트도 열려 있습니다. 광고로 트래픽이 늘기 전에
  한 번 점검할 가치가 있습니다.

**나중에 실제로 지우기로 한다면 순서**
1. 라우터에서 죽은 라우트를 먼저 제거(코드는 남긴 채) → 며칠 관측
2. 서비스 파일 삭제
