...
**증상:** README·개발·배포 문서에서 챌린지, 실천, AI 상담사 API가 현재 기능처럼 보인다.

**원인:** 2026-08-28 문장 되새기기 제품으로 전환한 뒤 역사 문서와 일부 개발 가이드를
보존했고, 레거시 Worker 파일과 D1 데이터도 의도적으로 남겼다.
...
## 4. 검색 도구가 설치되어 있다고 가정할 수 없다

**증상:** `rg` 실행이 `command not found`로 실패한다.

**원인:** 작업 환경에 ripgrep이 없을 수 있다.

**대응:** 설치 때문에 작업을 멈추지 말고 `find`, `grep`, `sed`로 대체한다. 단, zsh에서
`path`는 `PATH`와 연결된 특수 변수이므로 반복문 변수로 쓰지 않는다. `file_path`처럼
의도가 분명한 이름을 사용한다.
