...
**증상:** `docs/archive/`의 과거 실험·계획 문서에서 챌린지, 실천, AI 상담사 API가
현재 기능처럼 보인다.
...
## 9. 명령의 작업 디렉터리와 상대 경로를 함께 확인한다

**증상:** 파일이 실제로 있는데도 `find`나 `rg`가 경로를 찾지 못하거나 Markdown 링크 검사
대상이 0개로 나온다.

**원인:** 명령의 작업 디렉터리를 `workers/`처럼 하위 폴더로 지정하면서 저장소 루트 기준
상대 경로를 그대로 사용했다.

**대응:** 여러 패키지와 문서를 함께 검사하는 명령은 저장소 루트에서 실행한다. 특정
workspace에서 실행해야 하는 명령은 그 workspace 기준 경로만 사용한다. 검사 결과가 0개면
성공으로 보지 말고 예상 파일 수와 함께 확인한다.

샌드박스에서 Wrangler dry-run을 실행할 때 사용자 설정 폴더의 로그 쓰기 권한 오류가 나면
`WRANGLER_LOG_PATH=/tmp/dandani-wrangler.log`처럼 로그 경로를 쓰기 가능한 임시 위치로 지정한다.

## 10. UX 관찰과 E2E 테스트는 목적이 다르다

`automation/ux-check/check.mjs`는 운영 화면을 캡처해 사람이 UX를 관찰하기 위한 도구다.
운영 네트워크 상태에 의존하고 실패를 단언하지 않으므로 품질 게이트로 해석하지 않는다.

결정적 회귀 검사는 `npm run test:e2e`로 실행한다. 이 검사는 로컬 프런트엔드를 띄우고 문장
API를 브라우저에서 가로채므로 운영 D1을 읽거나 쓰지 않는다. `BASE_URL`을 지정하면 서버를
직접 띄우지 않으므로, 별도로 준비된 프리뷰 환경에도 같은 검사를 실행할 수 있다.
