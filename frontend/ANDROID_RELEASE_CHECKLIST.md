# Android 앱 출시 체크리스트

단단이 Android 앱을 Google Play Store에 출시하기 위한 체크리스트입니다.

## ✅ 완료된 항목

- [x] Capacitor 설정 완료
- [x] Android 플랫폼 추가 완료
- [x] 앱 아이콘 생성 완료 (모든 크기)
- [x] 스플래시 이미지 생성 완료 (모든 크기)
- [x] Application ID: `com.yetimates.dandani`
- [x] Version: `1.0` (versionCode: 1)

## 📋 출시 전 필수 체크리스트

### 1. 앱 정보 설정

#### Android 프로젝트 설정
- [x] **Application ID**: `com.yetimates.dandani` 확인
- [x] **Version Name**: `1.0` 확인
- [x] **Version Code**: `21` 확인
- [x] **Min SDK Version**: `23` 확인
- [x] **Target SDK Version**: `35` 확인

#### 앱 이름 및 아이콘
- [x] 앱 이름: `Dandani` (strings.xml)
- [x] 앱 아이콘: 모든 크기 생성 완료
- [x] 스플래시 이미지: 모든 크기 생성 완료

### 2. 서명 설정

#### 키스토어 생성
- [x] 키스토어 파일 생성 (`dandani-release-key.jks`)
- [x] 키스토어 비밀번호 안전하게 보관
- [x] 키 별칭 (alias): `dandani-key`

#### 서명 설정 파일
- [x] `android/keystore.properties` 파일 생성
- [x] `android/app/build.gradle`에 서명 설정 추가
- [x] 서명 테스트 빌드 성공 확인

### 3. 빌드 준비

#### 웹 앱 빌드
- [x] `npm run build` 성공 ✅
- [x] `npx cap sync android` 성공 ✅
- [x] 빌드 오류 없음 확인 ✅

#### Android 빌드
- [x] Android Studio에서 프로젝트 열기 성공 ✅
- [x] Gradle 동기화 완료 ✅
- [x] 의존성 다운로드 완료 ✅

### 4. Google Play Console 준비

#### 계정 설정
- [x] Google Play Developer 계정 생성 ($25)
- [x] 결제 정보 등록
- [x] 개발자 정보 입력

#### 앱 등록
- [x] 앱 만들기
- [x] 앱 이름: `Dandani`
- [x] 기본 언어: 영어
- [x] 앱 또는 게임: 앱
- [x] 무료 또는 유료: 무료

### 5. 스토어 리스팅

📖 **가이드**: [STORE_LISTING_GUIDE.md](../android/STORE_LISTING_GUIDE.md) 참고

#### Google Play Console 접근
- [ ] Google Play Console > 앱 선택 > 스토어 존재 > 스토어 리스팅

#### 필수 정보
- [x] **앱 이름**: `Dandani` ✅
- [x] **간단한 설명**: (최대 80자) - 가이드 참고
- [x] **전체 설명**: (최대 4000자) - 가이드 참고
- [x] **앱 아이콘**: 512x512 PNG 업로드 (준비 완료)
- [x] **기능 그래픽**: (선택사항, 1024x500)

#### 스크린샷
- [x] Phone 스크린샷: 최소 2개 (권장: 4-8개)
  - 크기: 1080 x 1920 이상
- [ ] 7-inch Tablet 스크린샷: (선택사항)
  - 크기: 1200 x 1920 이상
- [ ] 10-inch Tablet 스크린샷: (선택사항)
  - 크기: 1600 x 2560 이상

#### 카테고리 및 등급
- [x] 앱 카테고리 선택 (라이프스타일)
- [x] 콘텐츠 등급 설정 (전체이용가, PEGI 3)
- [x] 대상 고객 설정 (만 13세이상)

### 6. 개인정보 보호 및 권한

#### 권한 설정
- [x] 인터넷 권한 (AndroidManifest.xml)
- [x] 추가 권한이 필요한 경우 설명 추가

#### 개인정보 보호
- [x] 개인정보 보호 정책 URL: `https://dandani.yetimates.com/privacy` ✅
- [x] 데이터 안전 섹션 설정
- [x] 데이터 수집 여부 확인

### 7. 빌드 및 업로드

#### Release 빌드
- [x] 서명된 AAB 파일 생성 (`app-release.aab`) ✅
- [x] 빌드 성공 확인 ✅
- [x] 파일 크기 확인 (8.9MB, 최대 150MB 이하) ✅

#### 비공개 테스트 트랙 설정
⚠️ **중요**: 프로덕션 트랙으로 가려면 먼저 비공개 테스트를 완료해야 합니다.

- [ ] 비공개 테스트 트랙 생성
- [ ] 테스터 이메일 추가 (최소 1명 이상)
- [ ] AAB 파일 업로드
- [ ] 릴리스 노트 작성
- [ ] 검토 제출

### 8. 비공개 테스트 및 검증

#### 로컬 테스트
- [ ] Release APK 설치 테스트
- [ ] 실제 기기에서 실행 확인
- [ ] 주요 기능 동작 확인

#### 비공개 테스트 실행
- [ ] 테스터에게 테스트 링크 공유
- [ ] 테스터가 앱 설치 및 실행 확인
- [ ] 피드백 수집
- [ ] 버그 수정 (필요시)
- [ ] 비공개 테스트 완료 확인

### 9. 프로덕션 액세스 권한 신청

⚠️ **필수 조건**: 비공개 테스트를 완료해야 프로덕션 액세스 권한을 신청할 수 있습니다.

#### 프로덕션 액세스 권한 신청
- [ ] Google Play Console > 프로덕션 액세스 권한 신청
- [ ] 비공개 테스트에 관한 질문 답변:
  - [ ] 테스트 기간 및 테스터 수
  - [ ] 발견된 문제 및 해결 방법
  - [ ] 테스트 결과 요약
- [ ] 신청 제출 및 승인 대기

### 10. 프로덕션 출시

#### 최종 확인
- [ ] 프로덕션 액세스 권한 승인 완료 ✅
- [ ] 모든 체크리스트 완료
- [ ] 버그 없음 확인
- [ ] 스토어 리스팅 완료

#### 프로덕션 트랙 출시
- [ ] 프로덕션 트랙으로 승격
- [ ] 검토 제출
- [ ] 출시 완료 대기 (보통 1-3일)

## 🚀 빠른 시작

### 1단계: 웹 앱 빌드 및 동기화
```bash
cd frontend
npm run build
npx cap sync android
```

### 2단계: Android Studio 열기
```bash
npm run cap:android
```

### 3단계: 서명 설정
- 키스토어 생성
- keystore.properties 설정
- build.gradle 업데이트

### 4단계: Release 빌드
- Android Studio: `Build > Generate Signed Bundle / APK`
- AAB 파일 생성

### 5단계: 비공개 테스트
- 비공개 테스트 트랙 생성
- AAB 파일 업로드
- 테스터 추가 및 테스트 진행

### 6단계: 프로덕션 액세스 권한 신청
- 비공개 테스트 완료 후 프로덕션 액세스 권한 신청
- 질문 답변 및 승인 대기

### 7단계: 프로덕션 출시
- 프로덕션 트랙으로 승격
- 검토 제출 및 출시 완료 대기

## 📝 참고사항

- **AAB vs APK**: Google Play Store는 AAB를 권장합니다
- **버전 코드**: 매 업데이트마다 증가해야 합니다
- **비공개 테스트 필수**: 프로덕션 트랙으로 가려면 먼저 비공개 테스트를 완료해야 합니다
- **프로덕션 액세스 권한**: 비공개 테스트 완료 후 신청 가능하며, 질문에 답변해야 합니다
- **검토 시간**: 보통 1-3일 소요됩니다
- **테스트**: 비공개 테스트를 통해 충분히 테스트한 후 프로덕션으로 진행하세요

## 🔗 유용한 링크

- [Google Play Console](https://play.google.com/console)
- [Android 개발자 가이드](https://developer.android.com/)
- [Capacitor Android 가이드](https://capacitorjs.com/docs/android)

