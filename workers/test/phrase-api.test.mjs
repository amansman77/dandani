...
test('phrase API rejects missing identity, blank phrases, duplicates, and unknown phrases', async () => {
  ...
  assert.deepEqual(blank, { status: 400, body: { error: '문장을 입력해 주세요.' } });
  ...
  assert.deepEqual(duplicate, { status: 409, body: { error: '이미 사용 중인 문장이 있어요. 새로고침 후 다시 확인해 주세요.' } });

  const unknownLog = await responseJson(await handleRequest(request('/missing/log', { method: 'POST' }), { DB }));
  assert.deepEqual(unknownLog, { status: 404, body: { error: '문장을 찾을 수 없어요.' } });
});
