...
// phrase-service.js, phrase-mutations.js and core.logUserEvent; unsupported queries fail loudly.
...
function executeRun(state, sql, values) {
  ...
  if (sql.startsWith('INSERT OR IGNORE INTO daily_phrase_logs')) {
    const [id, logDate, phraseId, userId] = values;
    const active = state.phrases.some(item =>
      item.id === phraseId && item.user_id === userId && item.status === 'active');
    const exists = state.logs.some(item => item.phrase_id === phraseId && item.log_date === logDate);
    if (active && !exists) state.logs.push({ id, phrase_id: phraseId, user_id: userId, log_date: logDate });
    return { meta: { changes: active && !exists ? 1 : 0 } };
  }
