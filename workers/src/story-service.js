...
{
  "category": "다음 중 하나만: 일, 관계, 육아, 자기돌봄, 마음",
...
const VALID_CATEGORIES = ['일', '관계', '육아', '자기돌봄', '마음'];
...
export async function seedAiStories(env, count = 12, offset = 0) {
  ...
  for (let i = offset; i < offset + count; i += 1) {
    ...
    try {
      ...
      const category = VALID_CATEGORIES.includes(story.category) ? story.category : '마음';
      const id = generateId('story');
      await env.DB.prepare(`
        INSERT INTO stories (id, author_type, title, content, practice_title, practice_description, category)
        VALUES (?, 'ai', ?, ?, ?, ?, ?)
      `).bind(id, story.title, story.content, story.practice_title, story.practice_description || null, category).run();
...
export async function getStoryFeed(env) {
  const { results } = await env.DB.prepare(`
    SELECT id, title, category, author_type, created_at
    FROM stories
...
export async function getStoryDetail(env, storyId) {
  const story = await env.DB.prepare(`
    SELECT id, title, content, practice_title, practice_description, category, author_type, created_at
    FROM stories
