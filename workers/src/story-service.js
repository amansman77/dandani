...
{
  "emoji": "이야기의 주제를 한눈에 알려주는 이모지 하나 (예: 육아=👶, 일=💼, 운동=🏋️)",
  ...
  for (let i = offset; i < offset + count; i += 1) {
    ...
    try {
      ...
      await env.DB.prepare(`
        INSERT INTO stories (id, author_type, title, content, practice_title, practice_description, emoji)
        VALUES (?, 'ai', ?, ?, ?, ?, ?)
      `).bind(id, story.title, story.content, story.practice_title, story.practice_description || null, story.emoji || '🌱').run();
...
export async function getStoryFeed(env) {
  const { results } = await env.DB.prepare(`
    SELECT id, title, emoji, author_type, created_at
    FROM stories
...
export async function getStoryDetail(env, storyId) {
  const story = await env.DB.prepare(`
    SELECT id, title, content, practice_title, practice_description, emoji, author_type, created_at
    FROM stories
